...
class Mdl_Noticias extends MY_Model
{
    ...
    public function get_by_position($limit, $seccion = "", $position, $offset = 0)
    {
        ...
        $this->db->select('s.*,i.thumb300, i.thumbh120 as thumb1,i.thumbh120,i.thumbh80 as thumb2,i.thumbh50 as thumb3,s.created as time, (SELECT stories_stats.reads FROM stories_stats WHERE  stories_stats.story_id = s.id) AS lecturas, (SELECT categories.name FROM categories WHERE categories.id = s.category_id) AS category', FALSE);
