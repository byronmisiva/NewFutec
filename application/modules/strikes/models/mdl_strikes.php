...
class Mdl_strikes extends MY_Model
{
    ...
    function get_strikers($championship,$num=10){
        ...
        return	$this->db->query('SELECT p.last_name, p.first_name,p.nick, t.name, t.thumb_shield, t.shield,  COUNT(g.id) as goals, p.id, p.thumb,p.image, p.thumb220, p.position, p.height, p.born_place, UNIX_TIMESTAMP(NOW()) as n, UNIX_TIMESTAMP(p.birth) as b,
