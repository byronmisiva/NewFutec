<!--Sorteo entradas-->
<div class="col-md-12 separador10-xs margen0r" xmlns="http://www.w3.org/1999/html">
    <div class="panel-heading backcuadros">
        <h4 class="panel-title">
            Sorteo Entradas Selección
        </h4>
    </div>
    <div class=" margen0 col-md-12">
        <img class="img-responsive" src="/imagenes/entradas/noti-eliminatorias.jpg" alt="Entradas Ecuador Chile"/>        
    </div>
    <div class=" col-md-12">
        <iframe src="http://www.misiva.com.ec/concurso-entradas" width="100%" height="500px"frameborder="0" scrolling="no">

        </iframe>
    </div>
    <div class="col-md-12 text-right fondoazul  ">
        <a href="http://www.futbolecuador.com/concurso/terminos-concurso.pdf" target="_blank">Términos y condiciones</a>
    </div>
</div>
