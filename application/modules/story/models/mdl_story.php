...
class Mdl_story extends MY_Model
{
    ...
    function storys_by_tags($tag = "", $limit = RESULT_PAGE, $exclude = '', $offset = 0)
    {
        ...
        $this->db->select("s.id, s.category_id, s.title, s.subtitle, s.lead, s.body, s.created, (SELECT stories_stats.reads FROM stories_stats WHERE  stories_stats.story_id = s.id) AS lecturas,  i.thumb300, i.thumbh120, (SELECT categories.name FROM categories WHERE categories.id = s.category_id) AS category", FALSE);
