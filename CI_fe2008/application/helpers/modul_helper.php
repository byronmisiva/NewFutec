...
class Modul
{
    ...
    public function mod_Blog_ZonaFe($block)
    {
        ...
        $this->CI->load->model('blog');
        $data['noticias'] = $this->CI->blog->get_by_position(5, '63', 1);
        //elimimino la primera noticia
        array_shift($data['noticias']);
        if (count($data['noticias']) > 0)
            $this->CI->template->write_view($block, 'public/blog_zonafe', $data, FALSE);

    }

    public function mod_Noticias_ZonaFe($block, $num = 8)
    {
    ...
    function mod_Fuera_De_Juego($block)
    {
        $data = array();
        $this->CI->template->write_view($block, 'public/fueradejuego', $data, FALSE);
    }

    function mod_Fuera_De_Juego_Modulo($block)
    {
        $this->CI->load->model('profile');
        $data['profile'] = $this->CI->profile->get_last();
        $this->CI->template->write_view($block, 'public/fueradejuegomodulo', $data, FALSE);

    }
    //Concurso FE
    function mod_Twiter_Concurso($block)
    {
        $data = array();
        $this->CI->template->write_view($block, 'public/concursotwitter', $data, FALSE);
    }

    function mod_Twiter_Concurso_Modulo($block)
    {
        $this->CI->load->model('profile');
        $data['profile'] = $this->CI->profile->get_last();
        $this->CI->template->write_view($block, 'public/concursotwittermodulo', $data, FALSE);
    ...
    //Secciones news
    function mod_Contactos($block)
    {
        $data = array();
        $this->CI->template->write_view($block, 'public/contactos', $data, FALSE);
    }

    function mod_Staff($block)
    {
        $this->CI->load->model('profile');
        $data['profile'] = $this->CI->profile->get_last();
        $this->CI->template->write_view($block, 'public/staff', $data, FALSE);
    ...
    {
        $tag = $this->CI->uri->rsegment(3);

        //generar listado de keywords
        $this->CI->load->model('tag');

        if ($tag != false) {
            $metas = $this->CI->tag->get_story_tags($tag);
            $metas = str_replace(";", ",", $metas);
            $metas = substr($metas, 0, -2);
        } else {
            //todo definir que va por defecto en tags
            $metas = "futbolecuador, www.futbolecuador.com, futbol ecuador, futbol ecuador lo mejor del futbol ecuatoriano";
        }

        $this->CI->template->write('metas', $metas, TRUE);

        //generar description keywords
        $this->CI->load->model('story');

        if ($tag != false) {
            $descripcion = $this->CI->story->get($tag);
            $descripcion = $descripcion[0]->title;
        } else {
            //todo definir que va por defecto en tags
            $descripcion = "Noticias e información sobre clubes, jugadores, eliminatorias, copa libertadores y foro de discusión.";
        }
        $this->CI->template->write('descripcion', $descripcion, TRUE);
    }
    ...
    public function mod_Banner_Central_300_250($block)
    {

        $data['banner'] = '';
        $this->CI->template->write_view($block, 'public/banner_central_300_250', $data, FALSE);
    }
    ...
    public function mod_Statistics($block)
    {
        ...
        $this->CI->load->model('team');
        $this->CI->load->model('section');
        $dts = $this->CI->section->get($this->CI->uri->rsegment(3));
