...
class Scoreboards extends CI_Controller {
	...
	function list_matches_magazine(){
		$imagenes = array();
		$this->output->cache(CACHE_PARTIDOS);
		if( $this->config->item("encryption_key") == $this->uri->segment(3) ){
				
			$data['fondo_partido']="imagenes/match_center/barra_azul.png";
			if($this->uri->segment(4) == 'magazine')
				$data['fondo_partido']="imagenes/match_center/barra_negra.png";
			...
			$data['states'] = $this->states;
			$data['imagenes'] =  json_encode( $imagenes );
			$this->load->view( 'scoreboards/scoreboards_list_magazine', $data );
		}
	}
...
function game_all_magazine(){
		...
		if($this->config->item("encryption_key")==$this->uri->segment(4)){
			...
			$data['hora_cache'] = time();
			$data['fondo_partido']="imagenes/match_center/barra_azul.png";
			if($this->uri->segment(5) == 'magazine')
				$data['fondo_partido']="imagenes/match_center/barra_negra.png";
			$this->load->view('scoreboards/game_all_magazine',$data);
		}
	}
}
?>