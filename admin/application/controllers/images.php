...
class Images extends CI_Controller
{
        ...
        if (isset($_POST['submit'])) {
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($this->upload->do_upload('original')) {
                    ...
                    $_POST['thumb990'] = "imagenes/images/thumb990/" . $upload['file_name'];
                    ...
                    $this->images_thumb($dimension, "", $dimension['height']);
    ...
    function images_thumb($dimension, $prefix = '', $height = 0)
    {
...
//modificacion para don balon
        if ($height != 0) {
            $config['height'] = $height;
            $dimension['file_name'] = str_replace ( 'original' , 'thumb' . $prefix . $dimension['width'] , $dimension['file_name']  );
            $config['source_image'] = $dimension['file_name'];
            $config['new_image'] = '../imagenes/images/thumb' . $prefix . $dimension['width'] . '/';
            $config['x_axis'] = '1';
            $config['y_axis'] = '50';
    ...
    function no_thumb()
    {
        ...
        $datos = array();
        foreach ($result as $row) {
            $height = getimagesize($this->urlImagenes . $row->thumb640);
            $height = $height[1] + 80;
            $aux = "\n<table class='item' width='110' cellpadding='0' cellspacing='0'>\n";
            $aux .= "<tr><td colspan='4'>" . anchor('images/update/' . $row->id, img(array('src' => $this->urlImagenes . $row->thumb100, 'border' => '0')), array('title' => 'Cambiar Imagen')) . "</td><tr>\n";
