	...
	<table width='95%'>
		<tr>
			<td>
				...
				<option value="<?php echo $i?>" <?php if($row[0]->year==$i) echo " SELECTED "?>><?php echo $i?></option><?php }?>
				</select></td></tr>
				<tr><td>Ronda Activa:
							...
							<?php foreach($rnds->result() as $rou):?>
								<option value= <?='"'.$rou->id.'"'; if($row[0]->active_round==$rou->id) echo "SELECTED";?>><?=$rou->name?></option> 
							<?php endforeach;?>
						</select></td></tr>
				</table>
			</td>
