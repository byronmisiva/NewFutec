<div id='ultimos'>
	<table class='options' cellpadding="0" cellspacing="0" width="100%">
		<tr>
			...
			<td width='50%' onClick="ajax_update('ultimos','<?=base_url();?>championships/list_match_week/<?=$champ?>','<?=base_url();?>');">Próxima Fecha</td>
		</tr>
	</table>
	...
	if(count($partidos)>0){
		foreach($partidos as $row){
			...
			echo "<tr class='date'><td colspan='7' class='date'>".htmlentities(ucwords(strftime('%A, %d %B - %H:%M',$row->dm)))."</td></tr>\n";
