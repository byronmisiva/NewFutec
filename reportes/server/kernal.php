...
class kernal
{
   ...
   private function print_item_css($item){
      ...
      if(!$this->is_item_css_loaded($item)){
         ...
         // is the item a module?
            ...
            $success = $this->print_module_csslocal($item->id);
         }
    ...
    */
   private function print_module_csslocal($module_id){
      // do we have the required params?
   ...
   private function set_item_css_loaded($item){
      ...
      if(!$data){
        ...
        $type = $item->type;
        $id = $item->id;
        if (isset($data->$type->$id->loaded->css)) {
   ...
   private function set_item_javascript_loaded($item){
      ...
      if(!$data){
        ...
        $type = $item->type;
        $id = $item->id;
        if (isset($data->$type->$id->loaded->javascript)) {
